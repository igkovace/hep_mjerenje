...
from .const import (
    ...
    KEY_CONS_YESTERDAY,
    KEY_DIAG_CUR_ROWS,
    KEY_DIAG_FALLBACK_USED,
    KEY_DIAG_LAST_TS_P,
    KEY_DIAG_LAST_TS_R,
    KEY_DIAG_PREV_ROWS,
    KEY_DIAG_ROWS,
    KEY_DIAG_SKIPPED_MONTHS,
    KEY_DIAG_SUM_P,
    KEY_DIAG_SUM_R,
    KEY_EXP_MONTH,
    KEY_EXP_PREV_MONTH,
    ...
    async def _async_update_data(self) -> Dict:
        async with self._lock:
            try:
                ...
                _LOGGER.error("HEP login failed; cannot fetch measurements: %s", ex)
                raise
        ...
        # Yesterday
        cons_yday_kwh = sum(
            conv(r["val"]) for r in p_rows if r["ts"].date() == yesterday
        )
        exp_yday_kwh = sum(
            conv(r["val"]) for r in r_rows if r["ts"].date() == yesterday
        )
        ...
        if self._ytd_cache is None or (
            self._ytd_cache_date and self._ytd_cache_date.date() != today
        ):
            cons_year = 0.0
            exp_year = 0.0
            ptr = local_now.replace(month=1, day=1)
            safety = 0
    ...
    async def import_history(
        ...
        *,
        ...
        async with self._lock:
            ...
            return {
                "cons_total_kwh": self._persist["cons_total"],
                "exp_total_kwh": self._persist["exp_total"],
            }
